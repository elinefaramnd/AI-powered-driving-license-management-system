...
class DamagedReplacementController extends GetxController {
  ...
  Future<void> createDamagedReplacement(LicenseModel license) async {
    if (!isEligible(license)) {
      AppSnackbar.show("تنبيه", "الرخصة غير مؤهلة لإصدار بدل تالف");
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        AppSnackbar.show("نجاح", "تم إنشاء طلب بدل تالف بنجاح");
        Get.toNamed(
          "/upload_documents",
          arguments: {
            "applicationId": applicationId,
            "flow": "damaged_replacement",
          },
        );
      } else {
        AppSnackbar.show("خطأ", data["message"] ?? "فشل إنشاء طلب بدل تالف");
      }
    } catch (e) {
      AppSnackbar.show("خطأ", "فشل إنشاء طلب بدل تالف: $e");
    } finally {
